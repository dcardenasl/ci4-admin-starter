        ...
        <form method="post" action="<?= site_url('profile') ?>" class="mt-4 space-y-4">
            ...
            <div>
                ...
                <input id="avatar_url" name="avatar_url" type="url" value="<?= esc(old('avatar_url', $user['avatar_url'] ?? '')) ?>"
                    class="mt-1 w-full rounded-lg border px-3 py-2 <?= has_field_error('avatar_url') ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-brand-500 focus:ring-brand-500' ?>">
                <?= render_field_error('avatar_url') ?>
            </div>
            <button type="submit" class="rounded-lg bg-brand-600 text-white px-4 py-2 text-sm hover:bg-brand-700"><?= lang('Profile.saveChanges') ?></button>
        </form>
    </section>
        ...
        <form method="post" action="<?= site_url('profile/change-password') ?>" class="mt-4 space-y-4">
            ...
            <div>
                ...
                <input id="current_password" name="current_password" type="password" required
                    class="mt-1 w-full rounded-lg border px-3 py-2 <?= has_field_error('current_password') ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-brand-500 focus:ring-brand-500' ?>">
            ...
            <button type="submit" class="rounded-lg bg-brand-600 text-white px-4 py-2 text-sm hover:bg-brand-700"><?= lang('Profile.updatePassword') ?></button>
        </form>
