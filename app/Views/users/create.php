    ...
    <form method="post" action="<?= site_url('admin/users') ?>" class="mt-4 space-y-4">
        ...
        <div>
            ...
            <select id="role" name="role" required
                ...
                <option value="user" <?= old('role', 'user') === 'user' ? 'selected' : '' ?>><?= lang('Users.userRole') ?></option>
                <option value="admin" <?= old('role') === 'admin' ? 'selected' : '' ?>><?= lang('Users.adminRole') ?></option>
                <option value="superadmin" <?= old('role') === 'superadmin' ? 'selected' : '' ?>><?= lang('Users.superAdminRole') ?></option>
            </select>
