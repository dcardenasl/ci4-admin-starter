...
<section class="bg-white border border-gray-200 rounded-xl shadow-sm p-5"
    x-data="remoteTable({
        apiUrl: '<?= route_to('admin.iam.memberships.data') ?>',
        pageUrl: '<?= route_to('admin.iam.memberships') ?>',
        routes: {
            showBase: '<?= route_to('admin.iam.memberships') ?>',
            editBase: '<?= route_to('admin.iam.memberships') ?>'
        },
    ...
    <?= view('layouts/partials/table_toolbar', [
        ...
        'actionsView' => 'iam/memberships/partials/toolbar_actions',
    ]) ?>

    <?= view('layouts/partials/filter_panel', [
        'actionUrl'          => route_to('admin.iam.memberships'),
        'clearUrl'           => route_to('admin.iam.memberships'),
        'hasFilters'         => has_active_filters(request()->getGet(), ['limit' => '25']),
        'reactiveHasFilters' => true,
        'filterDefaults'     => ['limit' => '25'],
        'fieldsView'         => 'iam/memberships/partials/filters',
        'fieldsData'         => [
    ...
    <div class="mt-6 rounded-lg border border-dashed border-gray-300 bg-gray-50 p-4 text-sm text-gray-600" x-show="loading">
        <?= lang('Iam.app_user_memberships_loading') ?>
    </div>
    <div class="mt-6 rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700" x-show="error" x-text="errorMessage"></div>

    <template x-if="!loading && !error && rows.length === 0">
        <p class="mt-6 text-sm text-gray-500"><?= lang('Iam.app_user_memberships_no_results') ?></p>
    </template>
    <template x-if="!loading && !error && rows.length > 0">
